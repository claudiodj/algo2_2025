...
public class MainOld {
    ...
    private static void cargarLibros() {
        ...
        do {
            ...
            libroVO nuevoLibroVO = new libroVO();
            System.out.println("Ingrese nombre del libro: ");
            nuevoLibroVO.setNombre(teclado.nextLine());
            System.out.print("Ingrese autor: ");
            nuevoLibroVO.setAutor(teclado.nextLine());

            System.out.print("Ingrese editorial: ");
            nuevoLibroVO.setEditorial(teclado.nextLine());

            System.out.print("Ingrese ISBN (número): ");
            nuevoLibroVO.setIsbn(teclado.nextInt());

            teclado.nextLine(); // limpiar buffer

            System.out.print("Ingrese género: ");
            nuevoLibroVO.setGenero(teclado.nextLine());

            System.out.print("¿Está discontinuado? (s/n): ");
            if (teclado.nextLine().equalsIgnoreCase("s")) {
                nuevoLibroVO.setDiscontinuo(true);
            }
            //listaLibros.add(libro);

            libroDAO libroDAO = new libroDAO();

            int nro = libroDAO.insertarLibro(nuevoLibroVO);

            if (nro > 0) {
                System.out.println("Libro insertado correctamente con el id_libro: " + nro);
            } else {
                System.out.println("Libro NO insertado. Se produjo un error!");
            }
    ...
    private static void listarLibros(ArrayList<libroVO> listaLibros) {
    ...
    private static int buscarLibro(ArrayList<libroVO> listaLibros, String nombreBuscado) {
        ...
        if (listaLibros.size() == 0) {
            ...
            for (libroVO lib : listaLibros) {
                if (lib.getNombre().equals(nombreBuscado)) {
                    ...
                    // System.out.println("Encontrado! Indice en la lista: " +
                    // listaLibros.indexOf(lib));
                    // System.out.println(lib);
                } else {
    ...
    private static ArrayList<Integer> buscarPorAutor(ArrayList<libroVO> listaLibros, String nombreBuscado) {
    ...
    private static void listarAlumnos(ArrayList<alumnoVO> listaAlumnos) {
    ...
    private static void listarBibliotecarios(ArrayList<bibliotecarioVO> listaBibliotecarios) {
        ...
        if (listaBibliotecarios.size() == 0) {
            ...
            for (bibliotecarioVO bib : listaBibliotecarios) {
    ...
    private static void cargarDatosDesdeLosArchivos() {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(ARCHIVO_LIBROS))) {
            listaLibros = (ArrayList<libroVO>) ois.readObject();
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(ARCHIVO_ALUMNOS))) {
            listaAlumnos = (ArrayList<alumnoVO>) ois.readObject();
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(ARCHIVO_BIBLIOTECARIOS))) {
            listaBibliotecarios = (ArrayList<bibliotecarioVO>) ois.readObject();
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(ARCHIVO_PRESTAMOS))) {
            listaPrestamos = (ArrayList<prestamoVO>) ois.readObject();
    ...
    private static void prestarLibro() {
        ...
        prestamoVO prestamoLibro = new prestamoVO(listaBibliotecarios.get(indiceBiblio), listaLibros.get(indiceLibro),
    ...
    private static void listarPrestamos(ArrayList<prestamoVO> listaPrestamos) {
        ...
        if (listaPrestamos.size() == 0) {
            ...
            for (prestamoVO pre : listaPrestamos) {
                System.out.println("Indice en la lista: " + listaPrestamos.indexOf(pre));
                // System.out.println(pre);
                System.out.println("Bibliotecario que prestó : " + pre.getBibliotecario().getNombre() + ", "
                        + pre.getBibliotecario().getApellido());
                System.out.println(
                        "Alumno que recibió : " + pre.getAlumno().getNombre() + ", " + pre.getAlumno().getApellido());
    ...
    private static void listarPrestamosSinDevolucion(ArrayList<prestamoVO> listaPrestamos) {
        ...
        if (listaPrestamos.size() == 0) {
            ...
            for (prestamoVO pre : listaPrestamos) {
                if (pre.getFechaDevolucion() == null) {
                    ...
                    System.out.println("Bibliotecario que prestó : " + pre.getBibliotecario().getNombre() + ", "
                            + pre.getBibliotecario().getApellido());
                    System.out.println("Alumno que recibió : " + pre.getAlumno().getNombre() + ", "
                            + pre.getAlumno().getApellido());
                    System.out
                            .println("Libro : " + pre.getLibro().getNombre() + ", autor: " + pre.getLibro().getAutor());
    ...
    private static void revisarAntesDeSalir(int hash_anterior_lib, int hash_anterior_alu, int hash_anterior_biblio,
            int hash_anterior_pres) {
        ...
        if (hash_anterior_lib != hash_listaLibros_actual || hash_anterior_alu != hash_listaAlumnos_actual
                || hash_anterior_biblio != hash_listaBibliotecarios_actual
                || hash_anterior_pres != hash_listaPrestamos_actual) {
